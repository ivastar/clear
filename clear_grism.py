    ...
    if run_prep:
        ...
        for direct, grism in zip(direct_files, grism_files):
            init.prep_direct_grism_pair(direct_asn=direct, grism_asn=grism, 
                ...
                raw_path = paths['path_to_RAW'], mask_grow=8, scattered_light=False, final_scale = None,
...
def extract_clear(field, cat):
    ...
    for i in range(len(grism)):
        ...
        for id in [id for id in tab['ID'] if id in model.cat.id]:
            try:
                model.twod_spectrum(id=id, grow=1, miny=-36, maxy=None, CONTAMINATING_MAGLIMIT=23, refine=False, \
                                    verbose=False, force_refine_nearby=False, USE_REFERENCE_THUMB=True, \
...
def stack_clear(field, cat):
    ...
    for i in range(len(grism)):
        root = grism[i].split('-G102')[0]
        model = unicorn.reduce.process_GrismModel(root=root, grow_factor=2, growx=2, growy=2, MAG_LIMIT=24, \
        ...
        for id in tab['ID']:
            if (id in model.cat.id) and (not os.path.exists(field+'-G102_{}.2D.fits'.format(id))):
                try:
                    ...
                    spec = Stack2D(id=np.int(id), inverse=False, scale=[1,99], fcontam=2.,ref_wave = 1.05e4, \
                                   root=field+'-G102', search=field+'-*-*-G102', files=None, go=True, new_contam=False)
                except:
                    continue
